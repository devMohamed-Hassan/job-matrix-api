...
export class UserService {
  ...
  async uploadProfilePic(
    ...
    if (user.profilePic?.public_id) {
      const oldKey = user.profilePic.public_id;
  ...
  async uploadCoverPic(
    ...
    if (user.coverPic?.public_id) {
      const oldKey = user.coverPic.public_id;
      await this.s3Service.deleteImage(oldKey);
    }
  ...
  async deleteProfilePic(userId: string): Promise<UserDocument> {
    ...
    if (user.profilePic?.public_id) {
      await this.s3Service.deleteImage(user.profilePic.public_id);
    }
  ...
  async deleteCoverPic(userId: string): Promise<UserDocument> {
    ...
    if (user.coverPic?.public_id) {
      await this.s3Service.deleteImage(user.coverPic.public_id);
    }
